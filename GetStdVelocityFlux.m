...
function [StdVelocityFlux]=GetStdVelocityFlux(Widths,Rext,Xext,Yext,PIVx,PIVy,PIVu,PIVv)
...
%   hold on
  ...
  Number=0; 
  StdVelocityFlux=0;
  AvgVelocityFlux = GetAverageVelocityFlux(Widths,Rext,Xext,Yext,PIVx,PIVy,PIVu,PIVv);
...
%                     hold on
                    ...
                    Number=Number+1;
                    sumvarFlux=sumvarFlux+(dot(re_vect,v_vect)/norm(re_vect)-AvgVelocityFlux).^2;  
       ...
       if Number>0
          StdVelocityFlux=sqrt(sumvarFlux/(Number-1));
       end
   end
