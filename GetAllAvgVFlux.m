...
function AllAvgVFlux=GetAllAvgVFlux(reMin,reMax,Nre,Widths,fNbefaft,AllExtrusion,x,y,u_filtered,v_filtered)
               ...
               for f=1:2*fNbefaft+1
                   for r=1:Nre
                       ...
                       if ExtFrame-fNbefaft+f-1>=1  &&  ExtFrame-fNbefaft+f-1<=Totframes
                          ...
                          [AverageVelocityFlux(f,r)]=GetAverageVelocityFlux(Widths,Rext,Xext,Yext,PIVx,PIVy,PIVu,PIVv);
                       end
...
%       
%        subplot(3,3,j);
%        h= surf([reMin:(reMax-reMin)/(Nre-1):reMax],[-fNbefaft:fNbefaft],AllAvgVFlux{c,1}); xlabel('radius (pixel)'); ylabel('frame (10min)');title(['Extrusion frame ',num2str(AllExtrusion(j,1))]); caxis([-2 2.5]); 
%        if j==lengthAE
