...
function AllAvgSpeed=GetAllAvgSpeed(reMin,reMax,Nre,Widths,fNbefaft,AllExtrusion,x,y,u_filtered,v_filtered)
   ...
   if lengthAE>0
           for i=1:lengthAE
               ...
               %initiate
               AverageSpeed=zeros(2*fNbefaft+1,Nre);
               ...
               for f=1:2*fNbefaft+1
                   for r=1:Nre
                       if ExtFrame-fNbefaft+f-1>=1  &&  ExtFrame-fNbefaft+f-1<=Totframes
                              ...
                              [AverageSpeed(f,r)]=GetAverageVelocityParallel(Widths,Rext,Xext,Yext,PIVx,PIVy,PIVu,PIVv);
                       end
                        if ExtFrame-fNbefaft+f-1<1  &&  ExtFrame-fNbefaft+f-1>Totframes
                           AverageSpeed(f,r)=NaN;
                       end
                   end
               end
               %AllAvgVFlux
               %AverageVelocityFlux
               AllAvgSpeed=[AllAvgSpeed;AverageSpeed];
